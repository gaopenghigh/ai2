"""Day 01 加餐配图：真实模型的 KV Cache 演进
  ...
  fig6_kv_evolution.png   每 token KV 开销 与 1M 上下文总量（6 个真实模型）
...
def fig6_kv_evolution() -> None:
    ...
    # 颜色：KV 越小越"绿"
    order = per_kb.argsort()
    cols = [C["neutral"]] * len(models)
    palette = [C["ok"], C["memory"], C["accent"], C["capacity"], C["compute"], C["compute"]]
    for rank, idx in enumerate(order):
        cols[idx] = palette[min(rank, len(palette) - 1)]
    ...
    for i, v in enumerate(per_kb):
        ax1.text(v * 1.12, i, f"{v:.1f} KB", va="center", fontsize=9, fontweight="bold")
    ...
    for i, v in enumerate(at1m_gb):
        ax2.text(v * 1.12, i, f"{v:.0f} GB" if v >= 10 else f"{v:.1f} GB",
    ...
    fig.suptitle(f"图 6  KV Cache 的架构演进：三年间同样上下文的显存开销降了 {span:.0f} 倍",
                 fontsize=14, fontweight="bold")
    fig.tight_layout()
    fig.savefig(OUT / "fig6_kv_evolution.png")
    plt.close(fig)
