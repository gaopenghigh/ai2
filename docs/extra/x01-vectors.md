...
### 7.2 反过来那一步：输出层其实是同一张表

图 6 的最后一步写着“和词表里每个词做点积，取最像的那个”。那句话就是输出层。

**需求是这样的**：模型跑完 32 层，手里只有**一个 $d = 4096$ 维的向量** $h$。
但我们要的是“词表里 12.8 万个词，每个的分数是多少”：

$$
h \in \mathbb{R}^{4096} \quad\longrightarrow\quad \text{logits} \in \mathbb{R}^{128256}
$$

**最自然的做法：和每个词的向量做一次点积。**
假设词表里第 $j$ 个词有一个自己的向量 $w_j \in \mathbb{R}^{4096}$，那么

$$
\text{第 } j \text{ 个词的分数} = h \cdot w_j = \sum_{i=1}^{4096} h_i\, w_{j,i}
$$

**要做 12.8 万次点积。** 而 §7.5 会告诉你：一次一次地做是灾难。
所以把这 12.8 万个 $w_j$ **并排拼成矩阵的列**：

```
           w₁   w₂   w₃   …   w_V        ← 每一列是一个词的向量
         ┌────┬────┬────┬───┬────┐
         │    │    │    │   │    │
  4096 行 │    │    │    │   │    │        ← 所以矩阵形状是 (4096, 128256)
         │    │    │    │   │    │                          = (d, V)
         └────┴────┴────┴───┴────┘
                 128256 列
```

于是那 12.8 万次点积，变成**一次矩阵乘**：

$$
\underbrace{h}_{(1,\,d)} \times \underbrace{W}_{(d,\,V)} = \underbrace{\text{logits}}_{(1,\,V)}
$$

$$
\boxed{\ \text{输出层那个 } (d, V)\text{，就是「V 个词向量并排放着」}\ }
$$

#### 于是 weight tying 就很自然了

| | 形状 | 每一行 / 每一列是什么 |
|---|---|---|
| 输入 embedding 表 | $(V,\,d)$ | **每一行**是一个词的向量 |
| 输出层 $W$ | $(d,\,V)$ | **每一列**是一个词的向量 |

**互为转置 —— 本质上是同一张表。** 所以可以直接让它们共用一块内存：

```python
model.lm_head.weight = model.embed_tokens.weight   # 就这一行
```

> 顺带解答一个 PyTorch 的困惑：`nn.Linear(d, V)` 里存的 `weight` 张量，
> 形状是 `(V, d)` 而不是 `(d, V)`，前向时算的是 `x @ weight.T`。
> **所以它存下来的东西和 embedding 表形状完全一样**，赋值一行就绑定了。
> （为什么要存转置？**X02 会讲。**）

#### 这一步有多贵

decode 每生成一个 token，都要把这整个 $W$ 读一遍（fp16）：

$$
\text{FLOPs} = 2 \times 4096 \times 128256 = 1.05\ \text{GFLOP},\qquad
\text{Bytes} = 4096 \times 128256 \times 2 = 1.05\ \text{GB}
$$

$$
\text{算术强度} = \frac{1.05\times10^9}{1.05\times10^9} = \mathbf{1.0}\ \text{FLOP/Byte}
$$

**重度带宽受限**（4050 的平衡点是 125）。按 Day 01 的公式：

$$
\frac{1.05\ \text{GB}}{192\ \text{GB/s}} = \mathbf{5.5\ ms}
$$

> **光是“挑下一个词”这一步，就要 5.5 ms。**
> 而 Day 04 算出 7B INT4 的整个 TPOT 才 24 ms。
>
> 而且这一块通常**不能量化到 INT4** —— 量化输出层掉点明显，
> GGUF / AWQ 都会特意把它留在较高精度。
> 所以在一个全身 INT4 的模型里，**它的占比反而更突出**。
> 这就是大词表真实的代价，Week 2 会把这笔账算完。

### 7.3 这两张表有多大（接 Day 01 / Day 04 的显存账）
